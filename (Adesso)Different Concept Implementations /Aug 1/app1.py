...
def get_response(user_input, context_history, threshold=0.4):
    ...
    # Check for direct match in the CSV
    ...
    if context_response:
        ...
        # Update context history with the response
    ...
    # Check domain relevance if no match is found
    if is_domain_relevant(user_input):
        prompt = f"User asked: {user_input}. Please provide a helpful response related to women's heart health."
        logging.info(f"Prompt for Generative API (Domain Relevant): {prompt}")
    ...
    # Fallback response when no match is found and query is unrelated
