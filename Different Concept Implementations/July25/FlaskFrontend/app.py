...
#     "What is your diet preference (e.g., vegetarian, non-vegetarian)?",
#     "Do you smoke or consume alcohol? If yes, how often?"
# ]

# # Dictionary to store responses from the survey
# survey_responses = {}
...
#     # Check if a survey is ongoing
#     if session.get('survey_in_progress'):
#         survey_state = session.get('survey_state', 0)
        
#         # Save the previous response
#         survey_responses[survey_questions[survey_state]] = user_input

#         # Move to the next question
#         survey_state += 1

#         # Check if the survey has more questions
#         if survey_state < len(survey_questions):
#             session['survey_state'] = survey_state
#             next_question = survey_questions[survey_state]
#             return next_question, context_history
#         else:
#             # End the survey and provide a summary
#             session.pop('survey_in_progress', None)
#             session.pop('survey_state', None)
#             survey_summary = "Thank you for completing the survey! Here are your responses:\n" + "\n".join(
#                 f"{q}: {a}" for q, a in survey_responses.items()
#             )
#             return survey_summary, context_history

#     # Check if the user wants to start a survey
#     if user_input.lower() in ["start survey", "take survey", "survey"]:
#         session['survey_in_progress'] = True
#         session['survey_state'] = 0
#         return survey_questions[0], context_history
