...
def get_relevant_context(user_input, context_history):
    ...
    if any(keyword in user_input.lower() for keyword in follow_up_keywords) and len(context_history['history']) > 1:
        ...
        relevant_context = " ".join([h['user_input'] for h in context_history['history'][-2:]] + 
                                    [h['bot_response'] for h in context_history['history'][-2:]])  # Adjust range as needed
    else:
        # Use only the current query and the latest matching response
        relevant_context = " ".join([h['user_input'] for h in context_history['history'][-1:]] + 
                                    [h['bot_response'] for h in context_history['history'][-1:]])  # Adjust range as needed
