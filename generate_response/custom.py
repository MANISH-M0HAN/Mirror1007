...
def get_response(raw_user_input, threshold=0.3):
    ...
    spell_corrected_user_input = spell_checker.correct_spelling(raw_user_input, word_set)
    logging.info(f"After Spell Correct : {spell_corrected_user_input}")
    logging.info(f"Before Regex : {spell_corrected_user_input}")
    regexd_user_input = analyse_dataframe.preprocess_input(spell_corrected_user_input)
    logging.info(f"After Regex : {regexd_user_input}")
    
    context_responses = analyse_dataframe.find_best_context(regexd_user_input, threshold)
    if context_responses:
        combined_responses = []

        for context_response in context_responses:
            logging.info(f"Row is picked, now triggering match_columns()")
            column_response, ambiguous_query_flag = analyse_dataframe.match_columns(regexd_user_input, context_response)
            if column_response:
        ...
        request_response(raw_user_input, regexd_user_input, final_response)
        return final_response
    
    logging.info(f"3)Checking Domain relevance")
    if domain_check.is_domain_relevant(regexd_user_input):
        logging.info(f"Passing to AI method")
        prompt = f"User asked: {regexd_user_input}. Please provide a helpful response related to women's heart health."
        logging.info(f"Prompt for Generative API: {prompt}")
        response = default_messages.generate_response_with_placeholder(prompt)
        request_response(raw_user_input, regexd_user_input, response)
        return response
    ...
    request_response(raw_user_input, regexd_user_input, fallback_response)
