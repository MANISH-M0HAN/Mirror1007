...
def chatbot():
    ...
    # Find the best matching context for the current user input
    context = find_best_context(user_input)
    
    # Use only relevant context for the current query
    if context:
    ...
    # Append the user input and the response to the context history
    context_history.append(f"User: {user_input}")
    context_history.append(f"Bot: {response}")
    
    # Limit context history to the most recent 10 exchanges
