...
class Chatbot:
    ...
    def get_response(self, user_input, threshold=0.4):
        ...
        # Check if the user_input starts with a greeting word
        if any(user_input.lower().startswith(greeting) for greeting in greetings):
        ...
        if context is None:
            ...
            # Use generate_response to form a more coherent response
...
# You: sleep
# Corrected Text: sleep
# Corrected Text: sleep
# Query: sleep, Best Match Index: 9, Similarity: 0.6791478395462036
# Full Context: cholesterol high high cholesterol can lead to heart disease. it's important to maintain healthy levels through diet and medication if necessary. adequate sleep is vital for heart health. aim for 7-9 hours of quality sleep per night to support overall well-being.
# Traceback (most recent call last):
#   File "/Users/kishandahiya/Desktop/Adventure/June19/enhanced1715.py", line 114, in <module>
#     chatbot_interface()
#   File "/Users/kishandahiya/Desktop/Adventure/June19/enhanced1715.py", line 110, in chatbot_interface
#     response = chatbot.get_response(user_input)
#                ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "/Users/kishandahiya/Desktop/Adventure/June19/enhanced1715.py", line 64, in get_response
#     response = self.generate_response(context)
#                ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#   File "/Users/kishandahiya/Desktop/Adventure/June19/enhanced1715.py", line 46, in generate_response
#     outputs = self.model.generate(inputs['input_ids'], max_length=50, num_return_sequences=1, pad_token_id=self.tokenizer.eos_token_id)
#               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#            ^^^^^^^^^^^^^^^^^^^^^
#   File "/Library/Frameworks/Python.framework/Versions/3.12/lib/python3.12/site-packages/transformers/generation/utils.py", line 1648, in generate
#     self._validate_generated_length(generation_config, input_ids_length, has_default_max_length)
#   File "/Library/Frameworks/Python.framework/Versions/3.12/lib/python3.12/site-packages/transformers/generation/utils.py", line 1176, in _validate_generated_length
#     raise ValueError(
...
# kishandahiya@Kishans-MBP June19 % 